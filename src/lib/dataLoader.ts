...
// Tipos para identificação de conteúdo
...
// Classe principal para carregamento de dados
export class DataLoader {
  ...
  private async loadContentByType<T extends BaseContent>(
    ...
    try {
      const typeFolder = path.join(this.dataPath, type.toLowerCase());
      ...
      for (const file of jsonFiles) {
        const content = await this.loadJsonFile(path.join(type.toLowerCase(), file));
  ...
  public async loadContentById<T extends BaseContent>(
    ...
    try {
      if (type) {
        const content = await this.loadJsonFile(
          path.join(type.toLowerCase(), `${id}.json`)
        );
        if (content) {
        ...
        for (const searchType of types) {
          const content = await this.loadJsonFile(
            path.join(searchType.toLowerCase(), `${id}.json`)
          );
...
export async function findContentBySlug<T extends BaseContent>(
  ...
  return dataLoader.findBySlug<T>(slug, type);
}
...
export async function searchContent<T extends BaseContent>(
  ...
  return dataLoader.searchContent<T>(query, filters);
}